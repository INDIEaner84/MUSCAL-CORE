...
### Python-Pakete (Installation)
```bash
# NEU in v0.2: Ein Befehl für alle Dependencies
pip install -r requirements.txt
```

Enthaltene Pakete:
```
fastapi>=0.100.0    # API Control Plane
uvicorn>=0.23.0     # ASGI Server
websockets>=11.0    # Live-Streams
flask>=3.0.0        # Runtime API
flask-cors>=4.0.0   # CORS
requests>=2.31.0    # HTTP Client
numpy>=1.24.0       # Numerik
chromadb>=0.4.0     # Vektor-Index
playwright>=1.40.0  # Browser Engine
pyautogui>=0.9.54   # Desktop Tools
...
# 2. OS-Status prüfen (Smoketest):
python main_boot.py --status

# 3. Interaktive CLI:
python main.py

# 4. Loop-Demo (deterministisch):
...
- Erzeugt automatisch `storage/` Ordner (NEU in v0.2)
...
- `ObservationLoop`: Watchdog für Stuck/Drift
...
## 6. Ressourcen-Grenzen (NEU in v0.2)

Das System ist stabilisiert mit folgenden harten Limits:

| Ressource | Max | Eviction | Datei |
|-----------|-----|----------|-------|
| Graph Nodes | 5.000 | FIFO (ältester Timestamp) | `graph.py` |
| Graph Edges | 10.000 | FIFO | `graph.py` |
| Memory DB Einträge | 10.000 | ORDER BY id DESC | `memory.py` |
| Event History | 50.000 | POP(0) | `event_bus.py` |
| Kernel Safety Guard | 10.000 Nodes | Auto-Prune | `kernel.py` |

Diese Limits verhindern Speicherüberlauf bei längerer Laufzeit.
Siehe `TECHNICAL_MANUAL_v0.7.md` Abschnitt 3 für Details.
...
### Level 6: Stresstest (NEU in v0.2)
```bash
# 100-Iterationen Stresstest (Graph + Memory + Kernel):
python tests/stress_test.py
# Erwartet: 0 Crashes, deterministisches Ergebnis
```

## 9. Optimizer (LLVM-Style)
...
```
MUSCAL CORE/
├── main.py                        ← Simple CLI (Safe Entrypoint)
├── main_boot.py                   ← OS-Modus (Haupteinstieg)
├── api_server.py                  ← FastAPI Control Plane
├── bootstrap.py                   ← Swarm-Demo
├── config.py                      ← Konfiguration (lazy SESSION_ID)
├── event_bus.py                   ← EventBus (max 50.000 Events)
├── graph.py                       ← GraphState (max 5.000 Nodes, 10.000 Edges)
├── kernel.py                      ← Kernel-Pipeline (Safety Guard bei 10k Nodes)
├── memory.py                      ← Memory-Modul (max 10.000 Einträge, lazy DB)
├── mel.py                         ← Memory Execution Layer (lazy Runtime)
├── bridge.py                      ← Task → Tool Mapping
├── mkc_rules.py                   ← MKC Regeln (reset_state, DEFAULT_KEYWORDS)
├── muscal_os.py                   ← OS-Lifecycle
├── muscal_loop.py                 ← Autonomer LLM-Loop
├── os_config.py                   ← Deployment-Konfig
├── requirements.txt               ← NEU: Vollständige Dependency-Liste
├── Dockerfile                     ← Aktualisiert (requirements.txt, storage)
├── tests/
│   └── stress_test.py             ← NEU: 100-Iterationen Stresstest
├── runtime/
...
│       └── handoff.py             ← Handoff
├── docs/                          ← Wissen für RAG
├── storage/                       ← DBs + Snapshots (auto-created)
...
Feedback (Analyse + Anpassung)
  ↓
Memory (Speicherung + Pruning bei >10.000 Einträgen)
```

## 11. Bekannte Probleme

### Kritisch (v0.2 / MUSCAL CORE v0.7)

| # | Problem | Status |
|---|---------|--------|
| 1 | **Keine Tests** (0 Test-Dateien außer Stresstest) | ❌ Ungelöst |
| 2 | **~45% Stubs** (~80 Dateien <20 Zeilen) | ❌ Ungelöst |
| 3 | **SIGNAL_RULES Drift** — Confidence-Werte mutieren ohne Auto-Reset | ⚠️ Workaround: `reset_state()` |
| 4 | **FIFO Pruning** — Keine semantische Selektion (älteste ≠ unwichtigste) | ⚠️ Akzeptiert |
| 5 | **Keine DB-Migrationen** | ❌ Ungelöst |
...
|---------|---------|--------|
| `ModuleNotFoundError: <package>` | Paket fehlt | `pip install -r requirements.txt` |
...
| Graph wächst unbegrenzt (v0.2 fix) | Fehlerhafte Implementierung | Prüfe `graph.py` MAX_NODES = 5000 |
| ImportError bei `from memory import ...` | DB-Initialisierung | v0.2: Lazy-Init, `pip install -r requirements.txt` |
